...
if multi_webhook == "1":
    if len(webhook_urls) < 2:
        ctypes.windll.user32.MessageBoxW(0, "there's no reason to use multi-webhook... without multiple webhooks??", "bruh are you serious", 0)
    elif len(webhook_urls) > 14:
        if len(webhook_urls) > 49:
            ctypes.windll.user32.MessageBoxW(0, "you've gotta be doing this on purpose now... you don't need this many webhooks", "this is ridiculous", 0)
        else:
                                    ...
                                    if event == "NORMAL":
                                        ...
                                        if event == "GLITCHED":
                                            if heavenly_pop.get() == 1:
                                                ...
                                                webhook = discord_webhook.DiscordWebhook(url=webhookURL.get())
                                                embed = discord_webhook.DiscordEmbed(
                                                    description="[" + time.strftime('%H:%M:%S') + "]: Automatically popped " + heavenly_amt.get() + " Heavenly Potion IIs!", color="ff98dc")
                                                webhook.add_embed(embed)
                                                webhook.execute()
                                else:
                                    if event == "NORMAL":
...
def auradetection_toggle_update():
    if aura_detection.get() == 1:
        ctypes.windll.user32.MessageBoxW(0, "This feature is EXPERIMENTAL.\nThere are many limitations with aura detection.\n\nIt detects all auras that get equipped, so if you equip an aura yourself, it will get detected. Additionally, it will only detect auras that auto-equip.\n\nIt is also incapable of detecting dupes (for example, rolling Celestial with Celestial already equipped) or Overture: History, for some weird reason.", "Warning", 0)
...
ps_field = customtkinter.CTkEntry(tabview.tab("Webhook"), font=customtkinter.CTkFont(family="Segoe UI", size=20), width=300, textvariable=psURL)
...
youtube_link = customtkinter.CTkLabel(credits_frame, text="YouTube", font=("Segoe UI", 14, "underline"), text_color="dodgerblue", cursor="hand2")
...
support_link = customtkinter.CTkLabel(credits_frame, text="Discord", font=("Segoe UI", 14, "underline"), text_color="dodgerblue", cursor="hand2")
...
detectnotif_toggle = customtkinter.CTkCheckBox(tabview.tab("Macro"), text="Aura Notifications", font=customtkinter.CTkFont(family="Segoe UI", size=20), variable=aura_notif, command=auranotif_toggle_update)
detectnotif_toggle.grid(row=1, column=0, columnspan=2, padx=(10, 0), pady=(12, 0), sticky="w")

heavenly_toggle = customtkinter.CTkCheckBox(tabview.tab("Macro"), text="Auto-Heavenly [Experimental]", font=customtkinter.CTkFont(family="Segoe UI", size=20), variable=heavenly_pop, command=heavenly_toggle_update)
heavenly_toggle.grid(row=2, column=0, columnspan=2, padx=(10, 0), pady=(5, 0), sticky="w")

heavenly_field = customtkinter.CTkEntry(tabview.tab("Macro"), font=customtkinter.CTkFont(family="Segoe UI", size=20), width=50, textvariable=heavenly_amt)
